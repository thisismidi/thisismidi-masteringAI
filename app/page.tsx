      ...
      <div className="container" style={{maxWidth:'1000px', margin:'0 auto', padding:'20px'}}>
        <header style={{display:'flex', justifyContent:'space-between', marginBottom:'40px', alignItems:'center'}}>
          ...
          <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
            {user && <span style={{fontSize:'0.7rem', color:'var(--lbl)', marginRight:'10px'}}>TIER: <b style={{color: tier==='DEVELOPER'?'#eab308':'var(--txt)'}}>{tier}</b></span>}
              ...
              <div className="monitors">
                <div className="panel" style={{marginBottom:'20px'}}>
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'15px'}}>
                    ...
                    {file && (
                      <div style={{display:'flex', gap:'15px', alignItems:'center'}}>
                        <span style={{fontFamily:'monospace', fontSize:'0.75rem', color:'var(--lbl)', letterSpacing:'1px'}}>{formatTime(origTime)} / {formatTime(origDuration)}</span>
                  ...
                  <div style={{position:'relative', cursor:'pointer'}} onClick={(e) => handleSeek(e, origAudioRef, origDuration, false)}>
                    <canvas ref={origCanvas} width={700} height={120} style={{width:'100%', background:'var(--canvas-bg)', borderRadius:'8px', display:'block'}} />
                    {file && <div style={{position:'absolute', top:0, bottom:0, left:`${origDuration > 0 ? (origTime/origDuration)*100 : 0}%`, width:'2px', background:'var(--txt)', pointerEvents:'none'}} />}
                  </div>
                </div>
                ...
                <div className="panel">
                  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'15px'}}>
                    ...
                    {mastered && (
                      <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
                        <span style={{fontFamily:'monospace', fontSize:'0.75rem', color:'var(--lbl)', letterSpacing:'1px', marginRight:'5px'}}>{formatTime(mastTime)} / {formatTime(mastDuration)}</span>
                        ...
                        <a href={masterAudioUrl || '#'} download={`mastered_${file?.name || 'audio'}`} style={{textDecoration:'none'}}>
                          <button className="play-btn" style={{background:'#3b82f6', color:'#fff'}}>⬇ DOWNLOAD</button>
                        </a>
                      </div>
                    )}
                  </div>
                  <div style={{position:'relative', cursor: mastered ? 'pointer' : 'default'}} onClick={(e) => mastered && handleSeek(e, mastAudioRef, mastDuration, true)}>
                    <canvas ref={mastCanvas} width={700} height={120} style={{width:'100%', background:'var(--canvas-bg)', borderRadius:'8px', display:'block'}} />
                    {!mastered && <div style={{position:'absolute', top:0, left:0, right:0, bottom:0, display:'flex', alignItems:'center', justifyContent:'center', color:'var(--txt)', fontSize:'0.8rem', background:'var(--overlay)', borderRadius:'8px', fontWeight:'bold'}}>Waiting for mastering...</div>}
                    {mastered && <div style={{position:'absolute', top:0, bottom:0, left:`${mastDuration > 0 ? (mastTime/mastDuration)*100 : 0}%`, width:'2px', background:'var(--txt)', pointerEvents:'none'}} />}
                  </div>
                </div>
              </div>
